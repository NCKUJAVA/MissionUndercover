						...
						if (time > 0) {
							...
							if (time == 0) {
								if (gameStatus.equals("description")) {
									...
									/*
									 * int alives = 0; int undercoverCnt = 0; int civilianCnt = 0; for(int i = 0;i <
									 * alive.size();i++) { if (alive.get(i)) alives++; if
									 * (players.get(i).getIsUnderCover()) { undercoverCnt++; }else civilianCnt++; }
									 * if(civilianCnt <=2 undercoverCnt >=1) { /*TODO undercover win
									 */
									/*
									 * finishGame(); } else if (undercoverCnt ==0){ //civilian win finishGame(); }
									 * else { gameStatus = "description"; }
									 */
